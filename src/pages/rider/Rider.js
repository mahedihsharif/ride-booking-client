...
import { useRiderRidesInfoQuery } from "@/redux/features/rides/ride.api";
import { useState } from "react";
export default function RiderRidesTable() {
    ...
    const { data: ridesData, isLoading, isError, } = useRiderRidesInfoQuery({
        page: currentPage,
        limit,
    ...
    if (isError)
        return _jsx("div", { children: "Error fetching rides" });
    ...
    const handleSingleItem = (id) => {
        window.location.href = `/rider/rides/${id}`;
    };
    return (_jsxs("div", { children: [_jsxs("div", { className: "flex flex-wrap items-center gap-4 mb-6", children: [_jsxs("div", { children: [_jsx("label", { htmlFor: "statusFilter", className: "font-medium mr-2", children: "Status:" }), _jsx("select", { id: "statusFilter", value: statusFilter, onChange: (e) => {
                                    ...
                                    setCurrentPage(1);
                                }, className: "border rounded px-2 py-1 dark:bg-gray-800 dark:text-white" })] })] }), _jsx("div", { className: "overflow-x-auto", children: _jsxs(Table, { className: "min-w-full", children: [_jsx(TableHeader, { children: _jsxs(TableRow, { className: "bg-gray-100 dark:bg-gray-800", children: [_jsx(TableHead, { children: "Rider Name" }), _jsx(TableHead, { children: "Pickup Location" }), _jsx(TableHead, { children: "Destination" }), _jsx(TableHead, { children: "Fare" }), _jsx(TableHead, { children: "Driver Name" }), _jsx(TableHead, { children: "Status" }), _jsx(TableHead, { children: "Date" })] }) }), _jsx(TableBody, { children: ridesData?.data?.data?.map((ride, index) => (_jsxs(TableRow, { className: index % 2 === 0
                                    ? "bg-gray-50 dark:bg-gray-900 cursor-pointer"
                                    : "bg-white dark:bg-gray-800 cursor-pointer", onClick: () => handleSingleItem(ride._id), children: [_jsx(TableCell, { children: ride.rider?.name }), _jsx(TableCell, { children: ride.pickupLocation?.address }), _jsx(TableCell, { children: ride.destinationLocation?.address }), _jsxs(TableCell, { children: ["\u09F3", ride.fare] }), _jsx(TableCell, { children: ride.driver?.name || "N/A" }), _jsx(TableCell, { children: ride.status }), _jsx(TableCell, { children: ride?.createdAt
