...
export default function RequestDetails() {
    ...
    if (!ride) {
        return (_jsxs("div", { className: "flex flex-col items-center justify-center min-h-[400px] text-center p-6", children: [_jsx("h2", { className: "text-2xl font-bold text-muted-foreground mb-2", children: "Ride Information Not Available" }), _jsx("p", { className: "text-muted-foreground max-w-md", children: "We couldn't find any active ride with this ID. It may have been completed, cancelled, or the link might be incorrect." })] }));
    }
    return (_jsxs("div", { className: "flex items-center justify-center p-6 bg-background", children: [_jsxs(Card, { className: "w-full max-w-3xl shadow-xl rounded-2xl border", children: [_jsxs(CardHeader, { className: "flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3", children: [_jsxs("div", { children: [_jsxs(CardTitle, { className: "text-2xl font-bold flex items-center gap-2", children: [_jsx(Car, { className: "w-6 h-6 text-primary" }), "Ride Request to ", ride?.destinationLocation?.address] }), _jsxs("p", { className: "text-muted-foreground text-sm mt-1", children: ["Request ID: ", ride?._id] })] }), _jsx(Badge, { className: `px-4 py-1 text-sm
